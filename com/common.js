...
if(!window.tmlversion){
	window.tmlversion = 5.9;	//对以前的版本统一使用5.9
}
...
function dealCscmCertInfo(times)
{
	...
	try{
		...
		if(bsCertInfo == "SangforDefaultValue"){
			csCertInfo = base64decode(doQueryService("QUERY QSETTING RCERTINFO").deSerialize().note);
		}
		if(bsCertInfo != "SangforDefaultValue"){
			if(csCertInfo == ""){
...
function bulidMask(msg){
	if(!window.serviceMask){
		serviceMask = new XWindow({width:160,model:true,maskColor:"#F0F2F6",opacity:50,
							...
							content:"<div style='background:url(\"/com/images/svrloading.gif\") no-repeat;width:32px;height:32px;margin:auto;cursor:wait'></div><br/><span style='color:#4f2527;padding-top5px;'>正在初始化...</span>"
...
function showSettingLoading(){
	...
	if(!window.settingMask){
		settingMask = new XWindow({width:160,model:true,maskColor:"#ffffff",opacity:1,
							...
							contentStyle:{border:"0px solid #9fb3c3",backgroundColor:"transparent",color:'#fff',textAlign:"center"},
							content:"<img src='/com/images/setting_loading.gif' style='cursor:wait'/><br/><span style='color:#4f2527;padding-left:10px;'>页面加载中...</span>"
					});
	}
	settingMask.show();
	return settingMask;
...
function setSettingLoading(){
	if(window.settingMask){
		settingMask.setContent("<img src='/com/images/setting_loading.gif' style=''/><br/><span style='color:#4f2527;padding-left:10px;'>正在处理...</span>");
	}
}
...
XNewWindow.prototype = {
	init:function(){
		...
		var html = [
			...
			String.format("<a id='{0}' title='关闭'  class='xnewwindow_close'></a>",this.elId.closeBtnId),
			//"<br style='clear:both;'/>",
			"</div>",
